...
//static Mat _add(const int numInputs, Mat *inputs, const double *connectionWeights);
//static Mat _sub(const int numInputs, Mat *inputs, const double *connectionWeights);
//static Mat _mul(const int numInputs, Mat *inputs, const double *connectionWeights);
//static double _divide(const int numInputs, const double *inputs, const double *connectionWeights);
//Mat _and(const int numInputs, Mat *inputs, const double *connectionWeights);
...
DLL_EXPORT struct parameters *initialiseParameters(const int numInputs, const int numNodes, const int numOutputs, const int arity) {
	...
	strncpy_s(params->mutationTypeName, MUTATIONTYPENAMELENGTH, "probabilistic", _TRUNCATE);

	params->funcSet = (struct functionSet*)malloc(sizeof(struct functionSet));
	params->funcSet->numFunctions = 0;
	params->funcSet->num_smfunctions = 0;

	/* add functions to funcSet */
	add_functions(params);

	params->fitnessFunction = supervisedLearning_mcc;
	strncpy_s(params->fitnessFunctionName, FITNESSFUNCTIONNAMELENGTH,"supLearn_mcc", _TRUNCATE);

	params->selectionScheme = selectFittest;
	strncpy_s(params->selectionSchemeName,SELECTIONSCHEMENAMELENGTH, "selectFittest", _TRUNCATE);

	params->reproductionScheme = mutateRandomParent;
	strncpy_s(params->reproductionSchemeName, REPRODUCTIONSCHEMENAMELENGTH, "mutateRandomParent", _TRUNCATE);
...
DLL_EXPORT void addNodeFunction(struct parameters *params, char const *functionNames) {
	...
	strncpy_s(functionNamesAsArray, FUNCTIONNAMELENGTH * FUNCTIONSETSIZE, functionNames, _TRUNCATE);

	/* get the first function name */
	pch = strtok_s(functionNamesAsArray, ", ", &buf);
...
*/
DLL_EXPORT void addCustomNodeFunction(struct parameters *params, class Mat (*function)(const int numInputs, class Mat *inputs, int parameters[], int *to_do, const double *connectionWeights), char const *functionName, int maxNumInputs) {
	...
	strncpy_s(params->funcSet->functionNames[params->funcSet->numFunctions], FUNCTIONNAMELENGTH, functionName, _TRUNCATE);
...
void add_smfunction(struct parameters *params, void (*smfunction)(struct chromosome *chromo, int node_index) ) {

	/* add the given function to the function set */
	params->funcSet->smfunctions[params->funcSet->num_smfunctions] = smfunction;

	params->funcSet->num_smfunctions++;
}
...
static int addPresetFuctionToFunctionSet(struct parameters *params, char const *functionName) {
	...
	if (strncmp(functionName, "add", FUNCTIONNAMELENGTH) == 0) {
		...
		addCustomNodeFunction(params, _subtract, "sub", -1);
	}
...
DLL_EXPORT void setCustomFitnessFunction(struct parameters *params, double (*fitnessFunction)(struct parameters *params, struct chromosome *chromo, struct dataSet *data), char const *fitnessFunctionName) {
	...
	if (fitnessFunction == NULL) {
		...
		strncpy_s(params->fitnessFunctionName, FITNESSFUNCTIONNAMELENGTH, "supervisedLearning", _TRUNCATE);
	}
	else {
		params->fitnessFunction = fitnessFunction;
		strncpy_s(params->fitnessFunctionName, FITNESSFUNCTIONNAMELENGTH, fitnessFunctionName, _TRUNCATE);
	}
}
...
DLL_EXPORT void setCustomSelectionScheme(struct parameters *params, void (*selectionScheme)(struct parameters *params, struct chromosome **parents, struct chromosome **candidateChromos, int numParents, int numCandidateChromos), char const *selectionSchemeName) {
	...
	if (selectionScheme == NULL) {
		...
		strncpy_s(params->selectionSchemeName, SELECTIONSCHEMENAMELENGTH, selectionSchemeName, _TRUNCATE);
	}
}
...
DLL_EXPORT void setCustomReproductionScheme(struct parameters *params, void (*reproductionScheme)(struct parameters *params, struct chromosome **parents, struct chromosome **children, int numParents, int numChildren), char const *reproductionSchemeName) {
	...
	if (reproductionScheme == NULL) {
		...
		strncpy_s(params->reproductionSchemeName, REPRODUCTIONSCHEMENAMELENGTH, reproductionSchemeName, _TRUNCATE);
	}
}
...
DLL_EXPORT struct chromosome *initialiseChromosome(struct parameters *params) {
	...
	/* allocate memory for to do list */
	chromo->to_do = (int*)malloc(TODOLISTSIZE * sizeof(int));
	...
	chromo->outputValues = new Mat[(const int)params->numOutputs];

	/* Initialise each of the chromosomes nodes */
	for (i = 0; i < params->numNodes; i++){
		chromo->nodes[i] = initialiseNode(params->numInputs, params->numNodes, params->arity,params->funcSet->numFunctions,
	...
	chromo->nodeInputsHold = new Mat[(const int) params->arity];

	/* set the current input to initial value */
	chromo->current_input = 0;
...
DLL_EXPORT struct chromosome* initialiseChromosomeFromFile(char const *file) {
	...
	line = fgets(buffer, sizeof(buffer), fp);
	if (line == NULL) {/*error*/}
	record = strtok_s(line, ",\n",&buf);
	record = strtok_s(NULL, ",\n",&buf);

	/* for each function name */
	while ( record != NULL) {

		strncpy_s(funcName, FUNCTIONNAMELENGTH, record, _TRUNCATE);
	...
	for (i = 0; i < numNodes; i++) {
		...
		for (j = 0; j < arity; j++) {
			line = fgets(buffer, sizeof(buffer), fp);
			sscanf_s(line, "%d,%lf", &chromo->nodes[i]->relative_address[j], &chromo->nodes[i]->weights[j]);
		}
		for(int k = 0; k < PARAMETERSNUMBER; k++) {
			line = fgets(buffer, sizeof(buffer), fp);
			sscanf_s(line, "%d", &chromo->nodes[i]->func_params[k]);
		}
	}
...
DLL_EXPORT struct chromosome *initialiseChromosomeFromChromosome(struct chromosome *chromo) {
	...
	/* allocate memory for chromosome outputValues */
	chromoNew->outputValues = new Mat;

	/* allocate memory for to do list */
	chromoNew->to_do = (int*)malloc(TODOLISTSIZE * sizeof(int));
	...
	chromoNew->nodeInputsHold = new Mat;

	/* set the current input in the newly generated chromosome */
	chromoNew->current_input = chromo->current_input;
...
DLL_EXPORT void freeChromosome(struct chromosome *chromo) {
	...
	delete [] chromo->nodeInputsHold;
	free(chromo->funcSet);
	delete [] chromo->outputValues;
	free(chromo->nodes);
...
DLL_EXPORT void printChromosome(struct chromosome *chromo, int weights) {
	...
	for (i = 0; i < chromo->numNodes; i++) {
		...
		printf("(%d):\t%-15s\t", chromo->numInputs + i, chromo->funcSet->functionNames[chromo->nodes[i]->function]);
		...
		for (j = 0; j < getChromosomeNodeArity(chromo, i); j++) {
			...
			if (weights == 1) {
				printf("%-3d,%+.1f\t", chromo->nodes[i]->relative_address[j], chromo->nodes[i]->weights[j]);
			}
			else {
				printf("%-3d ", chromo->nodes[i]->relative_address[j]);
			}
		}
...
*/
DLL_EXPORT void executeChromosome(struct chromosome *chromo, Mat *inputs) {
	...
	for (i = 0; i < numActiveNodes; i++) {
		...
		for (j = 0; j < nodeArity; j++) {
			...
			nodeInputLocation = currentActiveNode + numInputs - chromo->nodes[currentActiveNode]->relative_address[j];

			if (nodeInputLocation < numInputs) {
				chromo->nodeInputsHold[j] = inputs[nodeInputLocation].clone();
			}
			else {
				chromo->nodeInputsHold[j] = chromo->nodes[nodeInputLocation - numInputs]->output[0].clone();
			}
		}
		...
		if(currentActiveNodeFuction < 4){
			chromo->nodes[currentActiveNode]->output[0] = chromo->funcSet->functions[currentActiveNodeFuction](chromo->numInputs, inputs,
				chromo->nodes[currentActiveNode]->func_params, chromo->to_do, chromo->nodes[currentActiveNode]->weights).clone();
		}
		else{
			chromo->nodes[currentActiveNode]->output[0] = chromo->funcSet->functions[currentActiveNodeFuction](nodeArity, chromo->nodeInputsHold,
				chromo->nodes[currentActiveNode]->func_params, chromo->to_do, chromo->nodes[currentActiveNode]->weights).clone();
		}

		restore_node_index(chromo->to_do, currentActiveNode + numInputs);
	}
	...
	for (i = 0; i < numOutputs; i++) {
		...
		if (chromo->outputNodes[i] < numInputs) {
			chromo->outputValues[i] = inputs[chromo->outputNodes[i]].clone();
		}
		else {
			chromo->outputValues[i] = chromo->nodes[chromo->outputNodes[i] - numInputs]->output[0].clone();
		}
	}
}

void restore_node_index(int *to_do, int node_index){

	int num = to_do[2];

	if(to_do[num] >= 0 && num < 2){
		to_do[num + 3] = node_index;
		to_do[2]++;
	}
}
...
/*
	...
	has been called
*/
DLL_EXPORT Mat getChromosomeOutput(struct chromosome *chromo, int output) {
...
/*
	used to access the chromosome node values after 
	has been called
*/
DLL_EXPORT Mat getChromosomeNodeValue(struct chromosome *chromo, int node) {
	...
	return chromo->nodes[node]->output[0];
}
...
DLL_EXPORT void saveChromosome(struct chromosome *chromo, char const *fileName) {
	...
	for (i = 0; i < chromo->numNodes; i++) {
		...
		for (j = 0; j < chromo->arity; j++) {
			fprintf(fp, "%d,%f\n", chromo->nodes[i]->relative_address[j], chromo->nodes[i]->weights[j]);
		}

		for(int k = 0; k < PARAMETERSNUMBER; k++){
			fprintf(fp, "%d\n", chromo->nodes[i]->func_params[k]);
		}
	}
...
DLL_EXPORT void saveChromosomeDot(struct chromosome *chromo, int weights, char const *fileName) {
	...
	for (i = 0; i < getNumChromosomeNodes(chromo); i++) {
		...
		if (chromo->nodes[i]->active == 1) {
			strncpy_s(colour, 20, "black", _TRUNCATE);
		}
		else {
			strncpy_s(colour, 20, "lightgrey", _TRUNCATE);
		}
		...
		for (j = 0; j < getChromosomeNodeArity(chromo, i); j++) {
			...
			if (weights == 1) {
				_snprintf_s(weight, 20, _TRUNCATE, "%.2f",chromo->nodes[i]->weights[j]);
			}
			else {
				_snprintf_s(weight, 20, _TRUNCATE, " (%d)", j);
			}


			fprintf(fp, "node%d -> node%d [label=\"%s\", labelfontcolor=%s, fontcolor=%s, bold=true, color=%s];\n",
				chromo->nodes[i]->relative_address[j], i + getNumChromosomeInputs(chromo), weight, colour, colour, colour);
		}
	}
...
static void saveChromosomeLatexRecursive(struct chromosome *chromo, int index, FILE *fp) {
	...
	if (strncmp(chromo->funcSet->functionNames[chromo->nodes[index - chromo->numInputs]->function], "add", FUNCTIONNAMELENGTH) == 0 ) {
		...
		saveChromosomeLatexRecursive(chromo, chromo->nodes[index - chromo->numInputs]->relative_address[0], fp);

		for (i = 1; i < getChromosomeNodeArity(chromo, index - chromo->numInputs); i++) {

			fprintf(fp, " + ");

			saveChromosomeLatexRecursive(chromo, chromo->nodes[index - chromo->numInputs]->relative_address[i], fp);
		}
...
void execute_todo(parameters *params, chromosome *chromo) {

	int func_num;

	for(int i = 0; i < 2; i++) {

	    func_num = chromo->to_do[i];
		if(func_num >= 0){
			params->funcSet->smfunctions[func_num](chromo, chromo->to_do[i + 3]);
		}
    }
}
		...
		if (chromo->nodes[i]->active == 0) {
			...
			for (j = 0; j < chromo->numNodes; j++) {
				for (k = 0; k < chromo->arity; k++) {
					...
					if (j - chromo->nodes[j]->relative_address[k] >= i + chromo->numInputs) {
						chromo->nodes[j]->relative_address[k]++;
					}
				}
			}
...
DLL_EXPORT void resetChromosome(struct chromosome *chromo) {
	...
	for (i = 0; i < chromo->numNodes; i++) {
		chromo->nodes[i]->output[0] = Mat::zeros(chromo->nodes[i]->output[0].cols, chromo->nodes[i]->output[0].rows, CV_8UC1);
	}
}
...
DLL_EXPORT void copyChromosome(struct chromosome *chromoDest, struct chromosome *chromoSrc) {
	...
	if (chromoDest->numNodes != chromoSrc->numNodes) {
		freeChromosome(chromoDest);
		chromoDest = initialiseChromosomeFromChromosome(chromoSrc);
	}
	...
	for(i = 0; i < TODOLISTSIZE; i++){
		chromoDest->to_do[i] = chromoSrc->to_do[i];
	}
	...
	/* copy current input */
	chromoDest->current_input = chromoSrc->current_input;
}
...
static void recursivelySetActiveNodes(struct chromosome *chromo, int nodeIndex) {
	...
	for (i = 0; i < chromo->nodes[nodeIndex - chromo->numInputs]->actArity; i++) {
		recursivelySetActiveNodes(chromo, nodeIndex - chromo->nodes[nodeIndex - chromo->numInputs]->relative_address[i]);
	}
}
...
*/
DLL_EXPORT struct dataSet *initialiseDataSetFromArrays(int numInputs, int numOutputs, int numSamples, Mat *inputs, Mat *outputs) {
	...
	data->inputData = (Mat**)malloc(data->numSamples * sizeof(Mat*));
	data->outputData = (Mat**)malloc(data->numSamples * sizeof(Mat*));

	for (i = 0; i < data->numSamples; i++) {

		data->inputData[i] = new Mat[(const int) numInputs];
		data->outputData[i] = new Mat[(const int) numOutputs];
...
DLL_EXPORT struct dataSet *initialiseDataSetFromFile(char const *file) {
	...
	while ( (line = fgets(buffer, sizeof(buffer), fp)) != NULL) {
		...
		if (lineNum == -1) {
			...
			sscanf_s(line, "%d,%d,%d", &(data->numInputs), &(data->numOutputs), &(data->numSamples));

			data->inputData = (Mat**)malloc(data->numSamples * sizeof(Mat*));
			data->outputData = (Mat**)malloc(data->numSamples * sizeof(Mat*));

			for (i = 0; i < data->numSamples; i++) {
				data->inputData[i] = (Mat*)malloc(data->numInputs * sizeof(Mat));
				data->outputData[i] = (Mat*)malloc(data->numOutputs * sizeof(Mat));
			}
		}
...
DLL_EXPORT void freeDataSet(struct dataSet *data) {
	...
	for (i = 0; i < data->numSamples; i++) {
		delete []data->inputData[i];
		delete []data->outputData[i];
	}
...
*/
DLL_EXPORT Mat *getDataSetSampleInputs(struct dataSet *data, int sample) {
...
*/
DLL_EXPORT Mat getDataSetSampleInput(struct dataSet *data, int sample, int input) {
...
*/
DLL_EXPORT Mat *getDataSetSampleOutputs(struct dataSet *data, int sample) {
...
*/
DLL_EXPORT Mat getDataSetSampleOutput(struct dataSet *data, int sample, int output) {
...
static void pointMutation(struct parameters *params, struct chromosome *chromo) {
	...
	numGenesToMutate = (int)floor((numGenes * params->mutationRate)+0.5);
	...
	for (i = 0; i < numGenesToMutate; i++) {
		...
		/* mutate node input gene */
			...
			chromo->nodes[nodeIndex]->relative_address[nodeInputIndex] = getRandomNodeInput(chromo->numInputs, chromo->numNodes, nodeIndex, params->recurrentConnectionProbability);
		}
...
static void singleMutation(struct parameters *params, struct chromosome *chromo) {
	...
	while (mutatedActive == 0) {
		...
		/* mutate node input gene */
			...
			previousGeneValue = nodeIndex - chromo->nodes[nodeIndex]->relative_address[nodeInputIndex];

			chromo->nodes[nodeIndex]->relative_address[nodeInputIndex] = getRandomNodeInput(chromo->numInputs, chromo->numNodes, nodeIndex, params->recurrentConnectionProbability);

			newGeneValue = nodeIndex - chromo->nodes[nodeIndex]->relative_address[nodeInputIndex];
...
static void probabilisticMutation(struct parameters *params, struct chromosome *chromo) {
	...
	for (i = 0; i < chromo->numNodes; i++) {
		...
		for (j = 0; j < params->arity; j++) {
			...
			if (randDecimal() <= params->mutationRate) {
				chromo->nodes[i]->relative_address[j] = getRandomNodeInput(chromo->numInputs, chromo->numNodes, i, params->recurrentConnectionProbability);
			}
...
static void probabilisticMutationOnlyActive(struct parameters *params, struct chromosome *chromo) {
	...
	for (i = 0; i < chromo->numActiveNodes; i++) {
		...
		for (j = 0; j < params->arity; j++) {
			...
			/* mutate the node input */
			if (randDecimal() <= params->mutationRate) {
				chromo->nodes[activeNode]->relative_address[j] = getRandomNodeInput(chromo->numInputs, chromo->numNodes, activeNode, params->recurrentConnectionProbability);
			}
...
DLL_EXPORT struct chromosome* runCGP(struct parameters *params, struct dataSet *data, int numGens) {
	...
	for (i = 0; i < params->mu; i++) {
		...
		//execute_todo(params, parentChromos[i]);
	}
	...
	for (gen = 0; gen < numGens; gen++) {
		...
		for (i = 0; i < params->lambda; i++) {
			...
			//execute_todo(params, childrenChromos[i]);
		}
...
static void copyFuctionSet(struct functionSet *funcSetDest, struct functionSet *funcSetSrc) {
	...
	for (i = 0; i < funcSetDest->numFunctions; i++) {
		strncpy_s(funcSetDest->functionNames[i], FUNCTIONNAMELENGTH, funcSetSrc->functionNames[i], _TRUNCATE);
...
*/
void copyNode(struct node *nodeDest, struct node *nodeSrc) {
	...
	for (i = 0; i < nodeSrc->maxArity; i++) {
		nodeDest->relative_address[i] = nodeSrc->relative_address[i];
		nodeDest->weights[i] = nodeSrc->weights[i];
	}

	for(j = 0; j < PARAMETERSNUMBER; j++){
		nodeDest->func_params[j] = nodeSrc->func_params[j];
	}
}
...
static void mutateRandomParent(struct parameters *params, struct chromosome **parents, struct chromosome **children, int numParents, int numChildren) {
	...
	for (i = 0; i < numChildren; i++) {
		...
		copyChromosome(children[i], parents[randInt(numParents - 1)]);
...
static struct node *initialiseNode(int numInputs, int numNodes, int arity, int numFunctions, double connectionWeightRange, double recurrentConnectionProbability, int nodePosition) {
	...
	for (i = 0; i < arity; i++) {
		n->relative_address[i] = getRandomNodeInput(numInputs, numNodes, nodePosition, recurrentConnectionProbability);
		n->weights[i] = getRandomConnectionWeight(connectionWeightRange);
	}

	/* set the output of the node to zero*/
	n->output = new Mat[1];
	n->output[0] = Mat::zeros(3, 3, CV_8UC1);	
...
void getRandomFunctionParams(int *func_params) {
	func_params[0] = randInt(255);
	func_params[1] = randInt(15) + 1;
	func_params[2] = randInt(15) + 1;
	func_params[3] = randInt(16);
	func_params[4] = randInt(16) - 8;
}
...
static int getRandomNodeInput(int numChromoInputs, int numNodes, int nodePosition, double recurrentConnectionProbability) {
	...
	/* pick any previous node including inputs */
		...
		input = randInt(numChromoInputs + nodePosition - 1) + 1;
	}
...
//	}
//	if(inputs[0].depth() != CV_8U){
//		inputs[0].convertTo(inputs[0], CV_8U);
//	}
//	if(inputs[1].depth() != CV_8U){
//		inputs[1].convertTo(inputs[1], CV_8U);
//	}
//	if(inputs[0].channels() != 1){
//		cvtColor(inputs[0], inputs[0], CV_BGR2GRAY, 1);
//	}
//	if(inputs[1].channels() != 1){
//	    cvtColor(inputs[1], inputs[1], CV_BGR2GRAY, 1);
//	}
//
//	for(int i = 0; i < inputs[0].rows; i++) {
//		p0 = inputs[0].ptr<uchar>(i);
//		p1 = inputs[1].ptr<uchar>(i);
//		p_out = output.ptr<uchar>(i);
//		for(int j = 0; j < inputs[1].cols; j++) {
//			if(p0[j] != 0 && p1[j] != 0) {
//				p_out[j] = (p0[j] + p1[j]) / 2;
//			}else if(p0[j] == 0 && p1[j] != 0){
//				p_out[j] = 0;
//			}else if(p0[j] == 0 && p1[j] != 0){
//				p_out[j] = 0;
//			}else{
...
static double supervisedLearning(struct parameters *params, struct chromosome *chromo, struct dataSet *data) {
	...
	for (i = 0 ; i < getNumDataSetSamples(data); i++) {
		...
		(chromo, getDataSetSampleInputs(data, i));
		...
		for (j = 0; j < getNumChromosomeOutputs(chromo); j++) {
			...
			if(output_mat.rows != target_mat.rows || output_mat.cols != target_mat.cols){
				resize(output_mat, output_mat, Size(target_mat.cols, target_mat.rows));
			}

			if(output_mat.type() != target_mat.type()){
				if(target_mat.depth() != CV_8U){
				    target_mat.convertTo(target_mat, CV_8U);
				}
				if(target_mat.depth() != CV_8U){
				    output_mat.convertTo(output_mat, CV_8U);
				}
				if(target_mat.channels() != 1){
				    cvtColor(target_mat, target_mat, CV_BGR2GRAY, 1);
				}
				if(output_mat.channels() != 1){
				    cvtColor(output_mat, output_mat, CV_BGR2GRAY, 1);
				}
			}

			absdiff(output_mat, target_mat, diff_mat);

			error += mean(diff_mat).val[0];
		}
	}
	return error;
}
