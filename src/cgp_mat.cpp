...
DLL_EXPORT struct parameters *initialiseParameters(const int numInputs, const int numNodes, const int numOutputs, const int arity) {
	...
	params->fitnessFunction = supervisedLearningMcc;
	strncpy(params->fitnessFunctionName, "supLearnMcc", FITNESSFUNCTIONNAMELENGTH);
...
*/
DLL_EXPORT struct chromosome* initialiseChromosomeFromFile(const char *file) {
	...
	for (i = 0; i < numNodes; i++) {
		...
		for (j = 0; j < arity; j++) {
			line = fgets(buffer, sizeof(buffer), fp);
			sscanf(line, "%d,%lf", &chromo->nodes[i]->relative_address[j], &chromo->nodes[i]->weights[j]);
		}
		for(int k = 0; k < PARAMETERSNUMBER; k++) {
			line = fgets(buffer, sizeof(buffer), fp);
			sscanf(line, "%d", &chromo->nodes[i]->func_params[k]);
		}
	}
...
DLL_EXPORT void executeChromosome(struct chromosome *chromo, Mat *inputs) {
	...
	for (i = 0; i < numActiveNodes; i++) {
		...
		//printf("%d ",chromo->nodes[currentActiveNode]->function);

		/* calculate the output of the active node under evaluation */
		if(currentActiveNodeFuction < 5){
			chromo->nodes[currentActiveNode]->output[0] = chromo->funcSet->functions[currentActiveNodeFuction](chromo->numInputs, inputs,chromo->nodes[currentActiveNode]->func_params, chromo->to_do, chromo->nodes[currentActiveNode]->weights).clone();
		}
		else{
	...
	for (i = 0; i < numOutputs; i++) {
		...
		if (chromo->outputNodes[i] < numInputs) {
			Mat tmp;
			threshold(inputs[chromo->outputNodes[i]], tmp, 1, 255, THRESH_BINARY);
			chromo->outputValues[i] = tmp.clone();
		}
		else {
			Mat tmp;
			threshold(chromo->nodes[chromo->outputNodes[i] - numInputs]->output[0], tmp, 1, 255, THRESH_BINARY);
			chromo->outputValues[i] = tmp.clone();
		}
	}
}
...
struct results* initialiseResults(struct parameters *params, int numRuns) {
	...
	/*
		Initialised chromosomes are returns from 
		runCGP and stored in a results structure.
...
*/
struct node *initialiseNode(int numInputs, int numNodes, int arity, int numFunctions, double connectionWeightRange, double recurrentConnectionProbability, int nodePosition) {
